...
- [x] **T21** Documents and results on the timeline; recent abnormal results on the summary card — two timeline kinds: `document`, on the date printed on it (midday IST), only once it scanned clean, under consent for documents; and `result`, one entry per lab set when the sample was collected, its values outside range first ("ALT (SGPT) 82 U/L high · 3 tests"), under consent for observations. Both link to the reports tab; the Doctors & treatment tab leaves them out. The summary card carries the five latest abnormal values from the results list, shown as glyph and word. `results.e2e-spec.ts` covers both, and that another hospital without consent sees neither
- [x] **T22** The documents category on the consent screen — already offered since T5: the consent form lists every clinical data category, documents included
- [x] **T23** The timeline performance test with documents and results in the volume — a lab report with a three-test LFT at every second encounter (1,020 documents, 3,060 lab values). **The tripwire fired:** another hospital's first page rose to p95 883 ms against the 750 ms budget. Profiling found the lab branch evaluating row-level security, and so consent, on every vital sign before discarding it (migration 0029: a partial index per observation category, p95 768 ms), and every branch reading the patient's whole history for one page. Each branch now reads its newest rows per record id through a time-ordered index and stops at the page size, exact because the page's entries are among each branch's newest (migration 0030 adds the missing prescription and allergy indexes). Now p95 599 ms (first page, cross-hospital), 236 ms (eight years back), 58 ms (own record) — faster than before SP4. The projection is still not needed

...
- [x] **T25** Unit: versioning rules, consent evaluation, cache encryption lifecycle — the clinical app gains a test runner (vitest with fake-indexeddb): encryption (5 tests), the offline cache lifecycle across sign-in, reload, reconnection and sign-out (9), and the shared list of cacheable reads (6). Consent evaluation is tested case by case as the application role, each case rolled back (`consent-evaluation.e2e-spec.ts`, 10): categories, both ends of a date range, open ranges, expiry, revocation, grantee only, no tenant, emergency access while it lasts, and dates read in India Standard Time. Pure helpers get their own spec (8). Versioning rules live in database triggers and stay covered by `clinical-rls.e2e-spec.ts` (immutability, correction)
- [x] **T26** Integration: every endpoint; RLS with and without consent; audit of cross-hospital reads — a scan of every controller route against the behavioural specs found five clinical routes uncalled; `clinical-coverage.e2e-spec.ts` (8) covers them, and vitals, notes, procedures and an encounter's prescriptions across hospitals without consent, under it with the consent named in the audit trail, and after revocation. The authorization suite still classifies every route. Eighteen SP1 routes (auth sessions, hospitals, staff) have no behavioural test yet; flagged as separate work
- [x] **T27** Timeline performance test at realistic synthetic volume — one patient seen fortnightly for ten years across two hospitals beside 300 others (2,040 encounters, 4,080 diagnoses, 6,120 prescriptions, 8,160 observations, 2,040 notes, 510 procedures); budget p95 750 ms per page. **The tripwire fired on first run:** another hospital's first page took p95 2,207 ms. EXPLAIN traced it to consent evaluation walking the merge-alias recursions for every row; migration 0020 gives never-merged patients a one-probe fast path, with merged patients unchanged. Now p95 520 ms (first page, cross-hospital), 233 ms (eight years back), 66 ms (own record). The projection is not needed yet; the test stays as the tripwire
- [x] **T28** Smoke: the Amlapitta scenario from `planning.md` §1, end to end — `amlapitta-scenario.e2e-spec.ts`, through the API in eight steps: curator approval, diagnosis with TM2 and advisory codes, two formulations over four months, linking at the second hospital, consent at its front desk, the gastroenterologist's timeline, problem list, prescriptions with start dates and durations, allergy banner and summary card, then revocation hiding it all with the attempt audited. Tests and reports join the scenario with SP4

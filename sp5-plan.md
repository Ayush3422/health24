...
- [ ] **T1** Migrations: `patient_account` rework, `patient_portal_access`, `otp_challenge`, `patient_session`; `app.current_patient_id` and the patient branch in every read policy; consent recorder by patient
- [ ] **T2** OTP service and SMS provider interface with the development provider; rate limits (DF1, DF2)
- [ ] **T3** Portal auth routes, patient token audience and guards (DF3); desk activation routes and screen in the clinical app (J1)
- [ ] **T4** Row-level security suite for the patient context; authorisation suite for patient and staff tokens on both surfaces
